...
离线优先、可持续成长的个人原生 App。当前版本为`0.8.0`，包含：

- 风格化动态名片与三维信号场。
- 实习总览、现场日志、作品、能力图谱四个内部子界面。
- 可旋转、缩放和点选的氢气纯度仪双数据集三维响应空间，以及联动二维切片和双重停止判定。
- 本地原始视频证据入口。
- 将现场日志组织成空间关系的三维证据轨迹。
- 全界面横向滑屏、几何翻转导航、浮动按键、卡片展开和连续页面转场。
- 本地个性称呼和隐藏系统栏、顶栏、底栏的沉浸模式。
...
- 下载线路：APK通过与更新清单相同的GitHub Pages域名分发，避免客户端直接连接GitHub Release。
- 定时更新：`JobScheduler`在有网络时周期检查资源包。
- 失败保护：请求失败、结构错误、摘要不一致或文件超限时不触碰当前活动版本。
- 隐私隔离：原始视频和个性称呼只保存在设备中。
